...
const CrearFactura = () => {
  ...
  useEffect(() => {
    const fetchProductosAPI = async () => {
      try {
        const response = await fetch(`http://vps-1915951-x.dattaweb.com:8090/api/v1/producto/cliente/2`, {
          method: 'GET',
          headers: {
  ...
  const handleConfirmarVenta = () => {
    const productosCompletos = productos.every(producto => producto.codigo && producto.descripcion && producto.cantidad > 0 && producto.precioVenta > 0);
  ...
  const confirmarGuardarPedido = async () => {
    const facturaVentaDetalles = productos.map(producto => ({
    ...
    const { descuentoGeneral, descuentoEspecial, tipoFactura, gravado, exento, iva, total } = factura;
  ...
  return (
    <div>
      ...
      <div className='container-principal'>
        ...
        <label>CUIT:</label>
        <input type="text" value={clienteDetails.cuit} id="input-read" readOnly />
        </div>
 <div className="input-row">
   <label>Condición de IVA:</label>
   <input type="text" value={clienteDetails.condicionIva} id="input-read" readOnly />
 </div>
 <div className="input-row">
   <label>Condición de Venta:</label>
   <input type="text" value={clienteDetails.condicionVenta} id="input-read" readOnly />
 </div>
 <div className="input-row">
   <label>Razón Social:</label>
   <input type="text" value={clienteDetails.razonSocial} id="input-read" readOnly />
 </div>
 <div className="input-row">
   <label>Domicilio:</label>
   <input type="text" value={clienteDetails.domicilio} id="input-read"readOnly />
 </div>
</div>
        ) :null}
      ...
      <span>${factura.total}</span>
        ...
        <div className="bottom-controls">
          <div className="button-container">
            <button onClick={handleConfirmarVenta} className="confirm-button">{isEditing ? 'Actualizar Pedido' : 'Crear Factura'}</button>
          </div>
        </div>
