...
const CrearPedidoTwo = () => {
  ...
  const handleProductoChange = (index, field, value) => {
    ...
    const selectedProducto = productosAPI.find(producto => producto[field] === value);
    if (selectedProducto) {
      ...
      {productos.map((producto, index) => (
        <div className="input-row" key={index}>
          <div className="input-container small" >
            ...
            <Autocomplete
              getItemValue={(item) => item.codigo}
              items={productosAPI}
              renderItem={(item, isHighlighted) =>
                <div key={item.codigo} style={{ background: isHighlighted ? 'lightgray' : 'white' }}>
                  {item.codigo}
                </div>
              }
              value={producto.codigo}
              onChange={(e) => handleProductoChange(index, 'codigo', e.target.value)}
              onSelect={(value) => handleProductoChange(index, 'codigo', value)}
            />
          </div>
          ...
          <div className="input-container large">
            ...
            <Autocomplete
              getItemValue={(item) => item.descripcion}
              items={productosAPI}
              renderItem={(item, isHighlighted) =>
                <div key={item.descripcion} style={{ background: isHighlighted ? 'lightgray' : 'white' }}>
                  {item.descripcion}
                </div>
              }
              value={producto.descripcion}
              onChange={(e) => handleProductoChange(index, 'descripcion', e.target.value)}
              onSelect={(value) => handleProductoChange(index, 'descripcion', value)}
            />
          </div>
